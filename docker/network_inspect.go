...
func (cm *onceNetworkInspectOp) Exec(ctx context.CancelContext, m metrics.Metrics) error {
	return cm.spell.Exec(ctx, m, cm.callback)
}

// Exec executes the image creation for the underline docker server pointed to.
func (cm *NetworkInspectOp) Exec(ctx context.CancelContext, m metrics.Metrics, callback NetworkInspectResponseCallback) error {
	if cm.client == nil {
